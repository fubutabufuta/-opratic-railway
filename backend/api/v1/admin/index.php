...
class AdminAPI
{
    ...
    private function getProviders()
    {
        ...
        $stmt = $this->conn->prepare("
            SELECT sp.*, u.name, u.phone, u.email,
    ...
    private function getSubscriptions()
    {
        $stmt = $this->conn->query("
            SELECT s.*, sp.name as package_name, sp.price, sp.duration_months,
                   srv.company_name, u.name as provider_name
            FROM subscriptions s
            JOIN subscription_packages sp ON s.package_id = sp.id
    ...
    {
        $stmt = $this->conn->query("
            SELECT qr.*, u.name as user_name, u.phone as user_phone,
                   v.brand, v.model, v.plate,
    ...
    private function getProviderRequests()
    {
        ...
        $stmt = $this->conn->query("
            SELECT pr.*, sp.company_name as provider_name, u.name as contact_person
